...
int main(int argc, char *argv[]) {
    ...
    for (int ITER = 0; ITER < MAX_ITER; ITER++) {
        ...
        for (long long i=0; i<localParticleNumber; i++) {
            ...
            double vel_base = sqrt(p->v[0]*p->v[0] + p->v[1]*p->v[1] + p->v[2]*p->v[2]) * epsfac_inv;
        ...
        message(rank, "%04d] %.5e %.10g %.10g %.10g %.10g %.10g %.10g %.10g %.10g %.10g %.10g %.10g %.10g %.10g %.10g %.10g %.10g %d\n", ITER, a, vel_base_sum, vel_psi_sum, vel_phi_sum, acc_base_sum, acc_phi_sum, acc_counter_sum, acc_dot_sum, acc_phi_tot_sum, sqrt(vel_base_sqsum), sqrt(vel_psi_sqsum), sqrt(vel_phi_sqsum), sqrt(acc_base_sqsum), sqrt(acc_phi_sqsum), sqrt(acc_counter_sqsum), sqrt(acc_dot_sqsum), sqrt(acc_phi_tot_sqsum), recompute);
